...
public class Unit : MapObject, IQPathUnit
{
    ...
    //private IQPathUnit _iqPathUnitImplementation;

    // Constructor
    public Unit(Hex hex, GameObject go) {
        MyHex = hex;
        GO = go;
        Name = "Test Unit";
        MapObjectType = "Unit";
    ...
    public int CostToEnterTile(IQPathTile sourceTile, IQPathTile destinationTile)
    {
        ...
        int cost = 1;
        if(destinationHex.ReliefName == "Hill")
            cost += 1;
        else if(destinationHex.ReliefName == "Mountain")
            cost += 2;

        if(destinationHex.FeatureName == "Forest" || destinationHex.FeatureName == "Jungle" || destinationHex.FeatureName == "Reef")
            cost += 1;

        if((destinationHex.ReliefName == "Depth" && sourceHex.ReliefName != "Depth") || (destinationHex.ReliefName != "Depth" && sourceHex.ReliefName == "Depth")) //Amphibious transition
