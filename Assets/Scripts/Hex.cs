...
public class Hex : IQPathTile {
	...
	public string ReliefName; // Water, Flat, Hill, Mountain
	public string BiomeName; // Coast, Ocean, Tropical, Savanna, Desert, Steppe, Temperate, Taiga, Tundra
	public string FeatureName; // Forest, Jungle
	...
	public string Description()
	{
		string log = ToString() + ". Elevation: " + Elevation +
		             ...
		             ", Moisture: " + Moisture +
		             ". " + BiomeName + " " + ReliefName;
		string description = BiomeName + " " + ReliefName;
		...
		foreach (Edge e in GetEdges())
		{
			if (e.River != null)
			{
				...
				break;
				string path = "";
				foreach (Edge bend in e.River.Path)
				{
					path += " [" + bend + "]";
				}
				Debug.Log("River path: " + path);
			}
		}
