...
//	    Random.InitState(833121);
    	...
    	// Pick terrain and vegetation depending of temperature and moisture
        IdentifyBiomes();
    ...
    void IdentifyBiomes()
    {
	    foreach (Hex h in hexes)
	    {
            ...
            if((h.ReliefName == "Plain" || h.ReliefName == "Hill") && (h.BiomeName != "Desert" && h.BiomeName != "Tundra")) {

                float forestScore = h.Moisture;

                if(h.BiomeName == "Tropical" || h.BiomeName == "Savanna") {
                    //Can spawn jungle
                    int jungleChance = (int)Mathf.Lerp( 0, 50,  forestScore );
                    if( Random.Range (0, 100) < jungleChance) {
                        h.FeatureName = "Jungle";
                    }
                    ...
                    if( Random.Range (0, 100) < forestChance) {
                        h.FeatureName = "Forest";
                    }
                }
            }
            else if(h.BiomeName == "Water" || h.BiomeName == "Ocean") {
                if(h.Elevation > -0.8) {
    ...
    void IdentifyTerrain_Old()
    {
	    for(int column = 0; column < numColumns; column ++) {
    		...
    		for(int row = 0; row < numRows; row ++) {
                ...
                //Indentify features (forests, jungle, reef...)
                if((h.ReliefName == "Plain" || h.ReliefName == "Hill") && (h.BiomeName == "Plain" || h.BiomeName == "Snow")) {

                    float jungleScore = h.Moisture + h.Temperature * 0.75f - 0.75f;
                    float forestScore = h.Moisture;

                    if(h.BiomeName == "Tropical" || h.BiomeName == "Desert") {
                        //Can spawn jungle
